...
class Canvas {
public:
    /// Constructs a Canvas instance with initial screen dimensions.
    ...
    /// Instantiates and adds a new UI element of type T to the canvas collection.
    ...
    /// Updates the canvas viewport dimensions and recalculates its base metrics.
    /// @param width New viewport width in pixels.
    /// @param height New viewport height in pixels.
    void setSize(int width, int height);

    /// Sorts all active elements by Z-index, computes the discrete UI scale factor,
...
private:
    ...
    std::vector<std::shared_ptr<UIElement>> m_elements; ///< Collection tracking all active UI elements owned by this canvas.
};