...
/// Handles gamepad polling, button histories, normalized triggers, and analog stick dead zones.
...
public:
    ...
    /// Initializes Gamepad Manager and hooks hotplug callbacks.
    void init();

    /// Polls gamepad hardware states and updates button state histories.
    /// @note Call once per frame at the start of the update pass.
    void update();

    /// Checks whether a gamepad with the given joystick ID is currently connected.
    ...
    /// Checks whether a gamepad button is currently held down.
    ...
    /// Checks whether a gamepad button was pressed during the current frame.
    /// @param button GLFW gamepad button constant (e.g., @c GLFW_GAMEPAD_BUTTON_A).
    /// @param gamepadID GLFW joystick identifier (e.g., @c GLFW_JOYSTICK_1).
    ...
    /// Checks whether a gamepad button was released during the current frame.
    /// @param button GLFW gamepad button constant (e.g., @c GLFW_GAMEPAD_BUTTON_A).
    /// @param gamepadID GLFW joystick identifier (e.g., @c GLFW_JOYSTICK_1).
    ...
    /// Gets a 2D direction vector for the Left Stick (-1.0 to 1.0) with scaled radial dead zone.
    ...
    /// Gets a 2D direction vector for the Right Stick (-1.0 to 1.0) with scaled radial dead zone.
    /// @param deadZone Radial dead zone threshold (0.0 to 1.0).
    /// @param gamepadID GLFW joystick identifier (e.g., @c GLFW_JOYSTICK_1).
    /// @return Normalized 2D analog stick vector.
    [[nodiscard]] glm::vec2 getRightStick(float deadZone = DEFAULT_DEAD_ZONE, int gamepadID = GLFW_JOYSTICK_1) const;

    /// Gets raw value of Axis for current or previous frame.
    ...
    /// Gets normalized Left Trigger press value.
    /// @param gamepadID GLFW joystick identifier (e.g., @c GLFW_JOYSTICK_1).
    ...
    /// Gets normalized Right Trigger press value.
    /// @param gamepadID GLFW joystick identifier (e.g., @c GLFW_JOYSTICK_1).
    /// @return Trigger position re-mapped to [0.0, 1.0] (0.0 = released, 1.0 = fully pressed).
...
private:
    ...
    /// Holds internal state and input history for a single gamepad.
    struct GamepadState {
    ...
    /// Applies smooth radial dead zone scaling to raw analog stick input.
