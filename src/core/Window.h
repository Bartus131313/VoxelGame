...
/// Wrapper around GLFW window lifecycle, OpenGL context creation, and frame operations.
class Window {
public:
    /// Creates and initializes a GLFW window and OpenGL context.
    /// @param width Initial window width in pixels.
    /// @param height Initial window height in pixels.
    /// @param title Text displayed in the window title bar.
    Window(int width, int height, std::string  title);

    /// Destroys the GLFW window handle and frees associated resources.
    ~Window();
    ...
    /// Checks whether the user or OS requested the window to close.
    /// @return @c true if window should close, @c false otherwise.
    [[nodiscard]] bool shouldClose() const;

    /// Swaps front and back rendering buffers and processes OS window events.
    /// @note Call once per frame at the end of the frame cycle.
    void update() const;

    /// Clears the framebuffer's color and depth buffers with a specified RGBA color.
